...
const GalleryDisabledNavClassName = 'gallery-nav-disabled';
...
class Gallery {
    ...
    manageHTML() {
        ...
        this.lineContainerNod = this.containerNode.querySelector(`.${GalleryLineContainerClassName}`);
        ...
        this.dotNode = this.dotsNode.querySelectorAll(`.${GalleryDotClassName}`);
    ...
    setParameters() {
        const coordsLineContainer = this.lineContainerNod.getBoundingClientRect();
    ...
    setEvents() {
        this.deboucedRecizeGallery = debounce(this.resizeGallery);
        window.addEventListener('resize', this.deboucedRecizeGallery);
    ...
    destroyEvents() {
        window.removeEventListener('resize', this.deboucedRecizeGallery);
    ...
    clickDots(evt) {
        ...
        let dotNumber;
        for (let i = 0; i < this.dotNode.length; i++) {
            if (this.dotNode[i] === dotNode) {
                dotNumber = i;
                break;
            }
    ...
    changeActiveDotClass() {
        for (let i = 0; i < this.dotNode.length; i++) {
            this.dotNode[i].classList.remove(GalleryDotActiveClassName);
        }
        this.dotNode[this.currentSlide].classList.add(GalleryDotActiveClassName);
    };
    
    changeDisableNav() {
        if (this.currentSlide <= 0) {
            this.navLeft.classList.add(GalleryDisabledNavClassName)
        } else  {
            this.navLeft.classList.remove(GalleryDisabledNavClassName)
        }
        if (this.currentSlide >= this.size - 1) {
            this.navRight.classList.add(GalleryDisabledNavClassName)
        } else  {
            this.navRight.classList.remove(GalleryDisabledNavClassName)
        }
    }
